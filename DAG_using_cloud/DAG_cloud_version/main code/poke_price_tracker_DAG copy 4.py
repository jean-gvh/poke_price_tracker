...
def web_scraping_format_pokemon_name():
    

    object_name2 = 'web_scraping_pokemon_names/web_scraping_names2.txt'
    bucket = client.get_bucket(bucket_base_data)
    blob = bucket.blob(object_name2)
    content = blob.download_as_string().decode("utf-8")

    # Diviser le contenu en lignes et supprimer les sauts de ligne
    lines = content.split("\n")
    web_scraping_names = [line.strip() for line in lines if line.strip()]
    return web_scraping_names
...
def scrape_pokemon_list(pokemon_names):
    ...
    for pokemon_name in pokemon_names:
        out = False
        soup_list = []  # Réinitialiser soup_list à chaque itération
        ...
        for soup in soup_list:
            if out:
                ...
                for item in results:
                    ...
                    card_name = title_span.text.replace('\n', '').strip() if title_span else None
                    ...
                    if title_tag_span:
                        ...
                        print(f" {pokemon_name}, date:{sold_date} nom:{card_name}")
                        if sold_date is not None:
                            # Supprimer les caractères indésirables de la date
                            sold_date = sold_date.replace('.', '')
                            sold_date = sold_date.replace('\n', '').strip()
                            sold_date = unidecode(sold_date)
                            sold_date = sold_date.replace("fevr",'feb')

                            # Convertir la date en objet datetime
                            try :
                                sold_date_datetime = datetime.strptime(sold_date, "%d %b %Y")
                                sold_date = sold_date_datetime.strftime('%Y-%m-%d')
                            except:
                                print("la date n'est pas celle d'aujourd'hui")
                            
                            # Comparer la date avec la date du jour
                            if sold_date != date_today :
                                out = True
                                break
                            ...
                            # Initialiser location avec None avant la condition
                                ...
                                vendor_name = vendor_name_element.text.replace('\n', '').strip() if vendor_name_element else None
                                ...
                                try :
                                    bids_numbers = bids_numbers.replace('\n', '').strip()
                                except:
                                    print("achat direct")
...
def chunk_list(lst, chunk_size):
        """Divise une liste en sous-listes de taille égale."""
        return [lst[i:i+chunk_size] for i in range(0, len(lst), chunk_size)]


def get_data():
    
    all_pokemon_names = web_scraping_format_pokemon_name()
    all_pokemon_names = chunk_list(all_pokemon_names,500)
...
def card_name_formatting(df : pd.DataFrame):
    ...
    web_scraping_names = web_scraping_format_pokemon_name()
    
    tuples = zip(web_scraping_names,pokemon_name_list)
    dict_poke_names = dict(tuples)

    def check_parts(row):
        scrapped_name_parts = row['scrapped_name'].split()  # Séparation du nom en parties
    
        # Vérifier si toutes les parties sont présentes et si 'carte' et 'fr' sont également présents dans card_name
        return (all(part in row['card_name'] for part in scrapped_name_parts) and
            'carte' in row['card_name'] and
            'fr' in row['card_name'])
    
    df['scrapped_name'] = df['scrapped_name'].map(dict_poke_names)

    df['card_name'] = df['card_name'].apply(unidecode)
    df['card_name'] = df['card_name'].str.lower()


    df = df[df.apply(check_parts, axis=1)]
    ...
    '''for row_index, row in df.iterrows():
        for pokemon_split_name in pokemon_list:
            conditions = all(part in row['card_name'] for part in pokemon_split_name) # le 1 est présent donc dracaufeu 1 99 
